...
void AOE::ClientRequestExecute()
{
	if (!clientExecuting_ && !node_->GetComponent<Silence>()->silenced_)
	{
		...
		SubscribeToEvent(E_TOUCHEND, HANDLER(AOE, TouchUp));
	}
}

void AOE::ServerExecute()
...
void AOE::TouchUp(StringHash eventType, VariantMap& eventData)
{
...
void AOE::SetGameMode()
{
	...
	emitterStartFX_->SetEffect(main_->cache_->GetResource<ParticleEffect>("Particle/aoe.xml"));
