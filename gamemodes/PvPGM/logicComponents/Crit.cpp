...
void Crit::ServerSync()
{
}

void Crit::ClientSync()
{
}

void Crit::ClientRequestExecute()
{
	if (!clientExecuting_)
	{
		...
		if (rigidBodies_.Size())
		{
			for (int x = 0; x < rigidBodies_.Size(); x++)
			{
				...
				if (noed != node_)
				{
					...
					int health = (int)(noed->GetComponent<Health>()->health_ * 0.25f);
					data_.WriteInt(health);//amount
...
void Crit::TouchUp(StringHash eventType, VariantMap& eventData)
{

}

void Crit::SetGameMode()
{
	...
	emitterEndFX_->SetEffect(main_->cache_->GetResource<ParticleEffect>("Particle/crit.xml"));
