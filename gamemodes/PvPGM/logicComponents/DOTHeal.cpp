...
DOTHeal::DOTHeal(Context* context) :
...
void DOTHeal::HandleUpdate(StringHash eventType, VariantMap& eventData)
{
	...
	/*if (!clientExecuting_ && !dothealed_)
	{
		UnsubscribeFromEvent(E_UPDATE);
	}*/
}

void DOTHeal::ServerSync()
{
	msg_.Clear();
	msg_.WriteInt(PvPGM::MSG_SynchronizeComponent_);
	msg_.WriteInt(clientID_);
	msg_.WriteString(mechanicID_);
	msg_.WriteBool(dothealed_);
	if (dothealed_)
	{
		msg_.WriteFloat(DOTHealElapsedTime_);
	}
	receiver_->SendMessage(MSG_GAMEMODEMSG, true, true, msg_);
}

void DOTHeal::ClientSync()
{
	dothealed_ = message_->ReadBool();
	if (dothealed_)
	{
		DOTHealElapsedTime_ = message_->ReadFloat();
		DOTHealElapsedTime_ += lagTime_;
		...
		//node_->GetComponent<DOTHeal>()->SubscribeToEvent(E_UPDATE, HANDLER(DOTHeal, HandleUpdate));
	}
}

void DOTHeal::ClientRequestExecute()
{
	if (!clientExecuting_ && !node_->GetComponent<Silence>()->silenced_)
	{
		...
		SubscribeToEvent(E_TOUCHEND, HANDLER(DOTHeal, TouchUp));
	}
}

void DOTHeal::ServerExecute()
{//todo casting times  (based on lag)
	...
	noed->GetComponent<DOTHeal>()->emitterEndFX_->SetEmitting(true);//server doesn't need to display gfx*/
	noed->GetComponent<DOTHeal>()->dothealed_ = true;
	noed->GetComponent<DOTHeal>()->DOTHealElapsedTime_ = 0.0f;
	noed->GetComponent<DOTHeal>()->DOTHealIntervalElapsedTime_ = 0.0f;
...
void DOTHeal::TouchUp(StringHash eventType, VariantMap& eventData)
{
...
void DOTHeal::SetGameMode()
{
	...
	emitterStartFX_->SetEffect(main_->cache_->GetResource<ParticleEffect>("Particle/dotheal.xml"));
...
void DOTHeal::HandleCleanseStatus(StringHash eventType, VariantMap& eventData)
{
